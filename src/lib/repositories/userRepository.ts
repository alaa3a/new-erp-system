...
export const userRepository = {
  findAll: () => (db.prepare('SELECT id, email, firstName, lastName, permissionIds, isActive, lastLoginAt, createdAt, updatedAt, version FROM users').all() as any[]).map(r => ({ ...r, permissionIds: JSON.parse(r.permissionIds || '[]'), isActive: r.isActive === 1 })),
  findById: (id: number) => { const r = db.prepare('SELECT id, email, firstName, lastName, permissionIds, isActive, lastLoginAt, createdAt, updatedAt, version FROM users WHERE id = ?').get(id) as any; return r ? { ...r, permissionIds: JSON.parse(r.permissionIds || '[]'), isActive: r.isActive === 1 } : null; },
  findByEmail: (email: string) => { const r = db.prepare('SELECT * FROM users WHERE email = ?').get(email) as any; return r ? { ...r, permissionIds: JSON.parse(r.permissionIds || '[]'), isActive: r.isActive === 1 } : null; },
  create: (data: any) => { const now = new Date().toISOString(); return db.prepare('INSERT INTO users (email, passwordHash, firstName, lastName, permissionIds, isActive, createdAt, updatedAt, version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)').run(data.email, data.passwordHash, data.firstName, data.lastName, JSON.stringify(data.permissionIds || []), data.isActive !== false ? 1 : 0, now, now).lastInsertRowid as number; },
  update: (id: number, data: any, version: number) => { const now = new Date().toISOString(); return db.prepare('UPDATE users SET firstName=?, lastName=?, isActive=?, permissionIds=?, updatedAt=?, version=version+1 WHERE id=? AND version=?').run(data.firstName, data.lastName, data.isActive !== false ? 1 : 0, JSON.stringify(data.permissionIds || []), now, id, version).changes > 0; },
  softDelete: (id: number, version: number) => db.prepare('UPDATE users SET isActive=0, updatedAt=?, version=version+1 WHERE id=? AND version=?').run(new Date().toISOString(), id, version).changes > 0,
