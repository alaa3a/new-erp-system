...
import {
  ...
  X, CheckCircle, BadgeCheck, BookOpen, Link2, Printer, Undo2,
} from 'lucide-react'
...
import PackingSlipModal from '@/components/invoices/PackingSlipModal'
...
function SalesPageContent() {
  ...
  // Task 45 — packing slip
  const [packingSlipTarget, setPackingSlipTarget] = useState<Invoice | null>(null)

  // Task 44 — return / credit note from invoice
  ...
  const fetchRefData = useCallback(async () => {
    try {
      ...
      if (ppRes.ok) {
        ...
        setPostingProfiles((json.data || []).filter((p: PostingProfile) => p.invoiceType === 'sales'))
      }
  ...
  return (
    <div className="space-y-6">
                    ...
                    <tr key={inv.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {(inv.status === 'posted' || inv.status === 'partial_paid' || inv.status === 'paid') && (
                            <>
                              <button onClick={() => setPackingSlipTarget(inv)}
                                className="p-1.5 rounded-lg text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 transition-colors"
                                title="Print packing slip"><Printer className="w-3.5 h-3.5" /></button>
                              <button onClick={() => setReturnTarget(inv)}
                                className="p-1.5 rounded-lg text-gray-400 hover:text-purple-500 hover:bg-purple-50 dark:hover:bg-purple-950/30 transition-colors"
                                title="Return / credit note"><Undo2 className="w-3.5 h-3.5" /></button>
                            </>
                          )}
      ...
      <InvoiceFormModal
        ...
        partnerPlaceholder="Select customer..."
        partnerSearchPlaceholder="Search customers..."
        partnerNotFoundLabel="No customers found"
        profilePlaceholder="Select posting profile..."
        title="New Sales Invoice"
        vatLabel="VAT Code"
        vatSearchPlaceholder="Search VAT codes..."
        vatNotFoundLabel="No VAT codes"
      />
      ...
      <InvoiceConfirmationModals
        ...
        cancelTitlePrefix="Cancel Invoice"
      />
      ...
      {packingSlipTarget && (
        <PackingSlipModal
          invoiceId={packingSlipTarget.id}
          onClose={() => setPackingSlipTarget(null)}
        />
      )}
